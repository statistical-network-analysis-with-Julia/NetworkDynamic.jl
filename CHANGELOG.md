...
- CI and documentation reconstruct sibling checkouts from package metadata,
...
- Continuous `rule=:all` queries and extraction now consider the union of
  adjacent or overlapping spells. Point queries at an excluded terminus no
  longer report activity. Read-only activity queries use no mutable global
  empty-vector cache and can run concurrently.
- Added qualified public `spell_active_at` and `elapsed_seconds` helpers for
  temporal consumers. Fixed-duration Dates values convert to seconds;
  calendar months/years require an origin date and are rejected.
