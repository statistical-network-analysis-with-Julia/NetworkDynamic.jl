    ...
    @testset "Continuous activity across adjacent spells" begin
        ...
        Threads.@threads for i in eachindex(answers)
            answers[i] = is_active(dnet, 0.5; vertex=1) &&
                         !is_active(dnet, 0.5; vertex=3)
        end
        @test all(answers)
    end

    @testset "Shared public time helpers" begin
        @test Base.ispublic(NetworkDynamic, :spell_active_at)
        @test Base.ispublic(NetworkDynamic, :elapsed_seconds)
        @test NetworkDynamic.spell_active_at(Spell(2.0, 2.0), 2.0)
        @test !NetworkDynamic.spell_active_at(Spell(1.0, 2.0), 2.0)
        seconds = NetworkDynamic.elapsed_seconds
